  ...
  module Billing #:nodoc:
    class CheckoutV2Gateway < Gateway
      ...
      def success_from(response)
        response['status'] == 'Pending'|| response['status'] == 'Declined' || response['response_summary'] == 'Approved' || response['approved'] == true || !response.key?('response_summary') && response.key?('action_id')
      end
